...
func makeperlinutil(repeat int) perlinutil {
	var p [512]int
	for i := 0; i < 512; i++ {
...
func (pln *perlinutil) octave(x, y, z float32, octaves int, persistence float32) float32 {
	var (
		total     float32 = 0
		frequency float32 = 1
		amplitude float32 = 1
	...
	for i := 0; i < octaves; i++ {
		total += pln.perlin(x*frequency, y*frequency, z*frequency) * amplitude
...
func (pln *perlinutil) perlin(x, y, z float32) float32 {
	if pln.repeat > 0 {
...
func (pln *perlinutil) inc(num int) int {
	num++
	if pln.repeat > 0 {
		num %= pln.repeat
	}
