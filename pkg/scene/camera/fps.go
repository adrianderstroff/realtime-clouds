...
// FPSCamera moves in the view direction while the viewing direction can be changed.
...
func (camera *FPSCamera) Rotate(theta, phi float32) {
	camera.theta += theta
	camera.phi += phi
...
func (camera *FPSCamera) GetOrtho() mgl32.Mat4 {
...
func (camera *FPSCamera) GetViewPerspective() mgl32.Mat4 {
...
func (camera *FPSCamera) SetPos(pos mgl32.Vec3) {
	camera.Pos = pos
	camera.Target = camera.Pos.Add(camera.dir)
}

// OnCursorPosMove is a callback handler that is called every time the cursor moves.
func (camera *FPSCamera) OnCursorPosMove(x, y, dx, dy float64) bool {
	dPhi := float32(-dx) / 2.0
...
func (camera *FPSCamera) OnMouseButtonPress(leftPressed, rightPressed bool) bool {
	return false
}

// OnMouseScroll is a callback handler that is called every time the mouse wheel moves.
func (camera *FPSCamera) OnMouseScroll(x, y float64) bool {
	return false
}

// OnKeyPress is a callback handler that is called every time a keyboard key is pressed.
func (camera *FPSCamera) OnKeyPress(key, action, mods int) bool {
